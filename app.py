#!/usr/bin/env python3
"""
Monarch V11 — Agnostic Autonomy Safety Kernel (Graph-Driven, FSM Policy, Replay-Deterministic)
...
What V10.5 already gave us
--------------------------
- Episodic memory (deterministic, bounded)
- FSM audit breadcrumbs
- Hardened intent TTL enforcement + invalidation events
- Risk explainability packets (per-feature contributions)

What V11 adds on top
--------------------
- Multi-asset safety orchestration (AssetRegistry + GlobalSafetyCoordinator)
- Composable safety monitors (pluggable, deterministic watchdogs)
- Deterministic inference engine over safety findings
- Light scenario generator for deterministic safety simulations

This file contains:

- MonarchKernelV10_5: single-asset safety kernel (as core engine)
- MonarchV11System: multi-asset orchestrator built from multiple V10.5 kernels
- CLI with two modes:
--version 10.5 → single asset demo
--version 11 → multi-asset V11 demo (default)
"""
...
def sleep(self, self_seconds: float) -> None:
time.sleep(self_seconds)
...
"""
Intent for downstream actuation layer. Not a direct actuator command.
"""
...
asset_constraints: Deque[Dict[str, Any]] = field(default_factory=lambda: deque(maxlen=20))
scenario_meta: Deque[Dict[str, Any]] = field(default_factory=lambda: deque(maxlen=20))
...
normalized: Deque[NormalizedTelemetry] = field(default_factory=lambda: deque(maxlen=5))
...
risk_explain: Deque[RiskExplainPacket] = field(default_factory=lambda: deque(maxlen=5))
...
muted_modules: Dict[str, str] = field(default_factory=dict)
...
"""Return (state, action, transition_label, breadcrumb)."""
r = risk.risk
tick = risk.tick
...
prior_entered = self._entered_state_tick
...
"risk_vector": {"risk": r, "anomaly": risk.anomaly},
}
...
def __init__(self, rng: random.Random, clock: Clock, asset_id: str = "asset_1"):
self.rng = rng
self.clock = clock
...
"""Feeds RawTelemetry from a prior journal in deterministic order."""
...
self._idx += 1 # ignore `tick` and replay recorded sequence
return rec
...
raise TypeError(f"Bad payload type for {evt_type}: expected {expected}, got {type(payload)}")
self.queue.append(Event(type=evt_type, ts=ts, payload=payload))

def drain(self, sandbox_callback: Callable[[str, Event, Callable[[Any], None]], None]) -> None:
while self.queue:
...
lateral_norm=self.cfg.normalization.clamp_norm("lane_offset_m", abs(raw.lane_offset_m)),
obstacle_norm=1.0 - self.cfg.normalization.clamp_norm("obstacle_m", raw.obstacle_m),
...
if pol_state == PolicyState.WATCH:
...
{"risk": pkt.risk, "anomaly": pkt.anomaly, "state": new_state.name, "ptype": ptype.value},
)
...
self._cooling_start: Dict[str, int] = {}

if self.state.mode == "SAFETY" and self.auto_commit:
raise RuntimeError("auto_commit must be False in SAFETY mode")
...
if self.auto_commit and self.state.mode == "DEMO" and proposal.ptype != ProposalType.SAFETY_CRITICAL:
self.commit(
proposal.id,
operator_id="demo_auto",
human_context={"auto": True, "reason": "demo_auto_commit"},
allow_cooling_override=True,
)
...
raise RuntimeError(f"Module dependency graph has a cycle or is incomplete: {order}")
...
# ===================================================================
# Monarch Kernel V10.5 (single-asset core)
# ===================================================================
...
self.adapter = adapter or DemoVehicleAdapter(self.rng, self.clock, asset_id=asset_id)
...
lambda mid: TelemetryNormalizer(mid, self.state, self.context, self.bus, self.audit, self.config),
)
self._register(
"anomaly_detector",
lambda mid: AnomalyDetector(mid, self.state, self.context, self.bus, self.audit, ZScoreAnomalyModel()),
)
self._register(
"risk_scorer",
lambda mid: RiskScorer(
mid, self.state, self.context, self.bus, self.audit, self.risk_kernel, self.feature_extractor
),
)
self._register(
"decision_gate",
lambda mid: DecisionGate(
mid, self.state, self.context, self.bus, self.audit, self.policy_fsm, self.config
),
)
...
mid, self.state, self.context, self.bus, self.audit, self.clock, self.safety_config, auto_commit
),
)
...
recs: List[RawTelemetry] = []
for e in journal.entries:
if e.event_type == "telemetry.raw":
recs.append(RawTelemetry(**dict(e.payload)))
return ReplayTelemetryAdapter(recs)

def _journal_event(self, event_type: str, ts: float, payload: Dict[str, Any]) -> None:
self.journal.append(Event(type=event_type, ts=ts, payload=payload), payload)
...
dur_ms = (time.monotonic() - start_perf) * 1000.0
if dur_ms > self.tick_budget_ms:
self._over_budget_streak += 1
self.audit.add(
"tick_budget",
f"Tick {tick} exceeded global budget: {dur_ms:.2f}ms (budget={self.tick_budget_ms}ms)",
{
"tick": tick,
"dur_ms": dur_ms,
...
if self.state.mode == "SAFETY" and self._over_budget_streak >= self.safety_config.max_overbudget_streak:
self.state.system_status = "DEGRADED"
...
# Episodic helpers for external callers
def remember_scenario(self, meta: Dict[str, Any]) -> None:
self.context.episodic.add("scenario_meta", meta)


# ===================================================================
# V11: Safety monitors, inference, multi-asset orchestration
# ===================================================================
...
"""Composable deterministic monitors that inspect heartbeats."""

def __init__(self, monitor_id: str):
self.monitor_id = monitor_id

@abstractmethod
def evaluate(self, asset_id: str, hb: Dict[str, Any]) -> SafetyFinding:
...


class ThermalMonitor(SafetyMonitor):
def __init__(self, warn: float = 0.6, hold: float = 0.8):
super().__init__("thermal")
self.warn = warn
self.hold = hold

def evaluate(self, asset_id: str, hb: Dict[str, Any]) -> SafetyFinding:
r = hb.get("latest_risk_explain")
level = SafetyLevel.NOMINAL
reason = "temp_nominal"
evidence: Dict[str, Any] = {}
if r and "details" in r and "temp_norm" in r["details"]:
feature = r["details"]["temp_norm"]["feature"]
evidence["temp_norm"] = feature
if feature >= self.hold:
level, reason = SafetyLevel.HOLD, "temp_high"
elif feature >= self.warn:
level, reason = SafetyLevel.WATCH, "temp_elevated"
return SafetyFinding(asset_id, self.monitor_id, level, reason, evidence)


class ObstacleMonitor(SafetyMonitor):
def __init__(self, warn: float = 0.6, stop: float = 0.85):
...
reason = "obstacle_ok"
evidence: Dict[str, Any] = {}
if r and "details" in r and "obstacle_norm" in r["details"]:
feature = r["details"]["obstacle_norm"]["feature"]
evidence["obstacle_norm"] = feature
if feature >= self.stop:
level, reason = SafetyLevel.STOP, "obstacle_critical"
elif feature >= self.warn:
level, reason = SafetyLevel.WATCH, "obstacle_close"
return SafetyFinding(asset_id, self.monitor_id, level, reason, evidence)


class CommsMonitor(SafetyMonitor):
def __init__(self, drop_warn: float = 0.1):
super().__init__("comms")
self.drop_warn = drop_warn
...
reason = "comms_ok"
evidence: Dict[str, Any] = {}
if r and "details" in r and "comms_drop" in r["details"]:
feature = r["details"]["comms_drop"]["feature"]
evidence["comms_drop"] = feature
if feature >= self.drop_warn:
level, reason = SafetyLevel.WATCH, "comms_drop_detected"
return SafetyFinding(asset_id, self.monitor_id, level, reason, evidence)


class DeterministicInferenceEngine:
"""
Simple, deterministic inference over safety findings.
Current policy: per asset, take max monitor level with rule-based downgrade.
"""

def infer_asset_level(self, findings: Iterable[SafetyFinding]) -> Tuple[SafetyLevel, List[SafetyFinding]]:
relevant = [f for f in findings if f.level != SafetyLevel.NOMINAL]
if not relevant:
return SafetyLevel.NOMINAL, []
worst_level = max(relevant, key=lambda f: f.level.value).level
return worst_level, relevant


@dataclass
class AssetRuntime:
asset_id: str
kernel: MonarchKernelV10_5
...
def register(self, asset_id: str, kernel: MonarchKernelV10_5) -> None:
if asset_id in self.assets:
raise ValueError(f"Duplicate asset_id {asset_id}")
self.assets[asset_id] = AssetRuntime(asset_id=asset_id, kernel=kernel)

def asset_ids(self) -> List[str]:
return sorted(self.assets.keys())

def kernels(self) -> List[MonarchKernelV10_5]:
return [rt.kernel for rt in self.assets.values()]


class ScenarioGenerator:
"""
Light deterministic scenario tagging. You can extend this to drive richer adapters.
"""

def __init__(self, name: str):
self.name = name

def apply(self, tick: int, runtime: AssetRuntime) -> None:
# For now, just log scenario metadata into episodic memory.
runtime.kernel.remember_scenario(
{"scenario": self.name, "tick": tick, "asset_id": runtime.asset_id}
)


class GlobalSafetyCoordinator:
"""
V11 multi-asset coordinator. Polls per-asset kernels, runs monitors, aggregates safety state.
"""

def __init__(
self,
registry: AssetRegistry,
monitors: Optional[List[SafetyMonitor]] = None,
inference_engine: Optional[DeterministicInferenceEngine] = None,
):
self.registry = registry
self.monitors = monitors or [ThermalMonitor(), ObstacleMonitor(), CommsMonitor()]
self.inference = inference_engine or DeterministicInferenceEngine()
self.asset_levels: Dict[str, SafetyLevel] = {aid: SafetyLevel.NOMINAL for aid in registry.asset_ids()}
self.asset_findings: Dict[str, List[SafetyFinding]] = {aid: [] for aid in registry.asset_ids()}

def evaluate(self, heartbeats: Dict[str, Dict[str, Any]]) -> None:
self.asset_levels.clear()
self.asset_findings.clear()
for asset_id, hb in heartbeats.items():
findings = [m.evaluate(asset_id, hb) for m in self.monitors]
level, relevant = self.inference.infer_asset_level(findings)
self.asset_levels[asset_id] = level
self.asset_findings[asset_id] = relevant
...
"findings": [
dict(
monitor=f.monitor_id,
level=f.level.name,
reason=f.reason,
evidence=f.evidence,
)
for f in self.asset_findings.get(aid, [])
],
}
for aid, lvl in self.asset_levels.items()
}


class MonarchV11System:
"""
Multi-asset orchestration over MonarchKernelV10_5 instances.
"""
...
mode: str = "DEMO",
tick_budget_ms: float = 20.0,
scenarios: Optional[List[ScenarioGenerator]] = None,
):
...
self.registry.register(aid, kernel)

self.coordinator = GlobalSafetyCoordinator(self.registry)
self.scenarios = scenarios or []
self.tick_count = 0

def tick(self) -> Dict[str, Any]:
self.tick_count += 1
# optional scenarios tagging
for scenario in self.scenarios:
for runtime in self.registry.assets.values():
scenario.apply(self.tick_count, runtime)

heartbeats: Dict[str, Dict[str, Any]] = {}
for aid, runtime in self.registry.assets.items():
runtime.kernel.tick()
heartbeats[aid] = runtime.kernel.heartbeat()

self.coordinator.evaluate(heartbeats)
summary = self.coordinator.summary()
return dict(
tick=self.tick_count,
assets=heartbeats,
global_safety=summary,
)
...
def run_v10_demo(args: argparse.Namespace) -> None:
kernel = MonarchKernelV10_5(
...
print("Monarch V10.5 single-asset demo")
...
inv = f" [invalidated:{li.get('invalidation_reason')}]" if li.get("invalidation_reason") else ""
print(
...
kernel.clock.sleep(args.interval)
...
def run_v11_demo(args: argparse.Namespace) -> None:
scenarios: List[ScenarioGenerator] = []
if args.scenario:
scenarios.append(ScenarioGenerator(args.scenario))

system = MonarchV11System(
...
print(f"Monarch V11 multi-asset demo — assets={args.assets}")
print("----------------------------------------")
...
global_safety = state["global_safety"]
for aid in sorted(state["assets"].keys()):
hb = state["assets"][aid]
lvl = global_safety[aid]["level"]
line = f" {aid}: status={hb['status']} policy={hb['policy_state']} global_level={lvl}"
if hb["latest_risk"]:
r = hb["latest_risk"]
line += f" risk={r['risk']:.3f} anom={r['anomaly']:.3f}"
print(line)
for finding in global_safety[aid]["findings"]:
print(
f" monitor={finding['monitor']} level={finding['level']} "
f"reason={finding['reason']} evidence={finding['evidence']}"
)
system.clock.sleep(args.interval)


def parse_args(argv=None) -> argparse.Namespace:
p = argparse.ArgumentParser("Monarch Kernel Demo (V10.5 / V11)")
p.add_argument("--version", type=str, default="11", choices=["10.5", "11"], help="Kernel version to demo")
...
p.add_argument("--json", action="store_true", help="Emit JSON per tick (machine-readable)")
# V11-specific
p.add_argument("--assets", type=int, default=3, help="Number of assets for V11 demo")
p.add_argument("--scenario", type=str, default="", help="Optional scenario name to tag episodic memory")
return p.parse_args(argv)


if __name__ == "__main__":
args = parse_args()
if args.version == "10.5":
run_v10_demo(args)
else:
run_v11_demo(args)
